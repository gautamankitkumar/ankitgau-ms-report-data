...
class Agent(object):
	def __init__(self, train_data, valid_data, model_paths, test_data=None, layer_nodes=[10, 10], activation='tanh', lr=1, max_iter=10, history_size=100, device=torch.device('cpu')):
		"""
		...
		self.optimizer = torch.optim.LBFGS(params, lr=lr, max_iter=max_iter, history_size=history_size)
		# self.optimizer = torch.optim.Adam(params, lr=lr, amsgrad=True)
		# self.optimizer = torch.optim.Adadelta(params, lr=lr)
		# self.optimizer = torch.optim.SGD(params, lr=lr, momentum=0, nesterov=False)
		...
		for epo in range(n_epoch):
			...
			for i_b in range(n_batchs):
				...
				def closure():
					...
					# print(f'epoch: {epo}, nrg_mae: {train_nrg_mae*1000} meV/atom, force_mae: {train_force_mae*1000} meV/AA, nrg_loss: {train_nrg_mae}, force_loss: {train_force_loss}')
					with open(log_name, 'a') as file:
						file.write(f'epoch: {epo}, nrg_mae: {train_nrg_mae*1000} meV/atom, force_mae: {train_force_mae*1000} meV/AA\r\n')
					# print(epo, train_nrg_mae.item()*1000)
					return train_loss
			...
			if epo % val_interval == 0:
				...
				print(f'validation: epoch: {epo}, nrg_mae: {valid_nrg_mae*1000} meV/atom, force_mae: {valid_force_mae*1000} meV/AA')
